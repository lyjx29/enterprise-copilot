...
| 里程碑 | 状态 | 备注 |
|---|---|---|
| M0 项目骨架 | 🔨 进行中 | 代码+本地验证通过（ruff/pytest/uvicorn 全绿）；**Docker compose 验收待 Docker 启动后补** |
| M1 核心多源 Agent 图 | ⬜ 未开始 | 依赖 M0 |
...
**当前进行中**：M0 项目骨架（本地验证已通过；Docker compose 验收待 Docker daemon 启动后补齐）。
**已完成的决策**：见 §4（ADR-1~13）。**未决问题**：见 §19（OQ-2 / OQ-4 待决）。
